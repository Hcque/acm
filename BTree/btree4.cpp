...
public:
    ...
    int pushdown(NODE* cur, const Record &target, Record &median, 
            NODE*& right)
    {
        if (!cur){
            median = target;
        ...
        res = pushdown(cur->branch[pos], target,extra, extra_branch);
        if (res == -1)
        {
            ...
            Record& median, NODE* &right)
    {
        ...
        if (pos <= mid)
        {
            ...
            right->count=cur->count-mid; cur->count = mid;
            push_in(cur, extra, extra_branch, pos);
        }
        else{
            mid++;
    ...
    int search_node(NODE *cur, const Record& target, int &pos)
    {
        pos = 0;
        if (!cur) return 0;
        if (target < cur->data[0])
        {
            pos = 0;
            return 0;
        }
        else if (target > cur->data[cur->count-1])
        {
            pos = cur->count;
            return 0;
        }
    ...
    int rec_search(NODE* cur, const Record& target)
    {
        ...
        int res = search_node(cur, target, pos);
        // cout << "============"<<pos << endl;
        if (!res) rec_search(cur->branch[pos], target);
        return res;
    }
